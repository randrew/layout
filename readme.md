...
which does not exist in MSVC. C++ operator overloading is used in MSVC to
simulate the feature in MSVC. The code generated by all three compilers is not
too different -- the `vector_size` extension is used mostly to keep the syntax
of the implementation easier to read.
